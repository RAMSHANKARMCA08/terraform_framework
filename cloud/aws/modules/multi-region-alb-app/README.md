...
DNS aliases, and a Route 53 PRIMARY/SECONDARY failover origin alias named
`origin-<application>.<domain>`. A shared CloudFront router should expose the
standard public `/<application>` path. The public hosted zone is looked up and
is never owned by this module.
